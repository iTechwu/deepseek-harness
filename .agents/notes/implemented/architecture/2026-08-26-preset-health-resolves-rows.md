...
**Discovery resolves each row it can prove will start, and imports nothing.** The resolve pass runs after the shape check in `packages/preset/agent-presets/src/discovery.ts`, so a malformed composition still answers with the shape reason. A package name is first looked up on disk — Node's own upward `node_modules` walk, stopping at `<package>/package.json` — then through an anchored CommonJS manifest resolver when that walk misses. A preset-relative or absolute specifier is statted instead, because URL resolution only joins those names and a preset shipping a file that was deleted would otherwise pass. Nothing is evaluated either way.

The disk lookup stays first for two reasons. It is the cheap path: a registered ESM loader hook turns every resolver call into a synchronous round-trip to the hooks thread, measured under the `tsx` hook the source launch installs at 2ms for a hit and 5ms for a miss against 0.055ms and 0.032ms on bare Node — 238ms of resolver time per roster read, where the walk answers the same 135 rows in 0.7ms. It also recognizes an installed package even when that package does not export its manifest. A miss falls back to `createRequire(harnessBase).resolve('<package>/package.json')`, whose explicit parent describes the deployment and whose narrow manifest request lets a launcher expose its installed-package overlay without importing plugin code. A Node builtin short-circuits ahead of both.

A deployment whose packages exist only behind an ESM hook remains unsupported: its launcher must also expose exact package-manifest requests to CommonJS resolution. The Desktop launcher does so while keeping profile-installed plugins ahead of its bundled fallback, and ordinary `dsh plugin install` still resolves entirely through the disk path because it puts every plugin beside the roster.
...
**Treat every disk miss as an unresolvable package.** Rejected. A launcher can supply the same package overlay to the Loader and an anchored CommonJS resolver without copying bundled packages into a writable profile. Calling that package broken contradicts what the mount can load and prevents existing sessions from resuming. The manifest fallback runs only for disk misses, so ordinary installs retain the measured fast path.
