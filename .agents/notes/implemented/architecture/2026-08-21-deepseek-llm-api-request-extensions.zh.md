...
指定基址的清单解析器优先于物理包搜索路径。桌面启动器可以选择较新的安装包，同时在磁盘上保留较旧的 profile 副本；绕过该解析器会报告 Loader 未选中的版本。清单导出缺失或私有时，回退到同一基址的物理查找，保留私有清单支持和树基址优先级。其他解析器错误仍会使请求失败。基于 Loader 的测试覆盖选中包遮蔽物理副本、私有清单和解析器钩子恢复。
