...
import { isContextWindowExceededError, isQuotaExceededError, LlmError, ProviderRequestId, REQUEST_BODY_TOO_LARGE_CODE } from '@deepseek-ai/dsh-llm'
...
export function providerError(raw: unknown, status: number | undefined, headers?: Headers): LlmError {
  ...
  // A gateway 413 carries an HTML page, not a provider error: the generic
  // message would only echo the status, so name the byte limit instead.
  const fallbackMessage = status === 413 && typeof error.message !== 'string'
    ? 'DeepSeek request body exceeds the gateway byte limit (HTTP 413)'
    : `DeepSeek Messages request failed (${status ?? 'stream error'})`
  ...
  else if (status === 413) code = REQUEST_BODY_TOO_LARGE_CODE
