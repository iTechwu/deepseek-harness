...
挂载 AppFrame 后，共享布局隔离会把键盘焦点保留在设置页内，并使周围应用进入 inert 状态。嵌套对话框或菜单优先处理 Escape，因此关闭它们时设置页保持打开。面板关闭后，焦点回到设置触发控件。
...
「桌面」分区仅在 Electron 预加载载体存在时出现，与侧边栏徽标共享同一更新观察：展示已安装版本、实时更新阶段，以及加入同一个壳拥有的更新操作的入口按钮。普通浏览器不会注册桌面分区。
