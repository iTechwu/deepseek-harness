...
The DeepSeek card edits the shared `llm-deepseek` endpoint, credentials, model catalog, and its `chat-completions` / `messages` protocol. When Messages is selected, the public endpoint placeholder is `https://api.deepseek.com/anthropic`. OpenAI Responses remains a `llm-pi-ai` route because the direct DeepSeek adapter does not implement that wire protocol.
