...
- **Truncated streams and empty completions are retryable errors, not silent successes.** Both adapters surface a provider stream that closes before its terminal marker as `STREAM_CLOSED`; they map a terminal `stop` finish that carried no content blocks to `finish {kind:'error'}` with the canonical `EMPTY_RESPONSE` code. The normal `dsh-llm-retry` policy retries both codes by default.
