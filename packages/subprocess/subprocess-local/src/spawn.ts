...
export function taskkillProcessTree(pid: number): void {
  ...
  spawnSync('taskkill', ['/PID', String(pid), '/T', '/F'], {
    stdio: 'ignore',
    // taskkill is a console binary; hide its window when invoked from a GUI host.
    windowsHide: true,
  })
}
...
export function spawnSubprocess(spec: SubprocessSpawnSpec, internals: SpawnInternals = {}): LocalSubprocessHandle {
  ...
  const child = spawn(program, args, {
    ...
    // Keep the child's console window out of the taskbar when a GUI host owns the tree.
    windowsHide: true,
  })
