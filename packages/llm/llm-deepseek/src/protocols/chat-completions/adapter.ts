...
import { attributionHeaders, contentHasImage, CONTEXT_WINDOW_EXCEEDED_CODE, isContextWindowExceededError, isQuotaExceededError, LlmAdapter, LlmError, ProviderRequestId, QUOTA_EXCEEDED_CODE, REQUEST_BODY_TOO_LARGE_CODE } from '@deepseek-ai/dsh-llm'
import type {
  ContentBlock,
  GenerateOptions,
...
import { catalogModelInfo, modelInfo, resolveRequestMaxTokens } from '../../common/model-info.ts'
...
export class ChatCompletionsAdapter extends LlmAdapter {
  ...
  private async * streamWithConnection(
    ...
    // GLM-5.3 public endpoints reject requests above 128K generated tokens, so
    // the wire cap resolves here where the catalog model is at hand.
    const configuredModel = connection.models.find(entry => entry.id === options.model)
    const requestOptions: GenerateOptions = {
      ...options,
      maxTokens: resolveRequestMaxTokens(options.model, options.maxTokens, configuredModel, connection.maxTokens),
    }
