...
两层 API 都提供会话控制：`Session.cancel()` 对应 `session/cancel`，`Session.resume()` 恢复持久化会话，`Session.set_approval_policy("ask"|"never")` 追加持久化策略事件。`DeepSeekHarness.run(..., environment={"DEEPSEEK_BASE_URL": "https://..."})` 可以传递不可变、无机密的逐会话端点覆盖；服务器会拒绝凭据和动态加载变量。单项审批提示仍需未来的 server-to-client 请求/响应扩展。
