...
With AppFrame mounted, the shared layout isolation keeps keyboard focus inside Settings and makes the surrounding application inert. A nested dialog or menu handles Escape before Settings, so closing it leaves Settings open. The settings trigger regains focus after the panel closes.
