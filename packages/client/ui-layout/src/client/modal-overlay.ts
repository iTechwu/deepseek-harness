...
/** Keep the frame background inert and keyboard focus inside the top dialog. */
  ...
  const syncModal = (): void => {
    ...
    if (nextDialog !== null) {
      ...
      queueMicrotask(() => {
        ...
        else { nextDialog.tabIndex = -1; nextDialog.focus({ preventScroll: true }) }
      })
      return
    }
    ...
    returnFocus = null
    queueMicrotask(() => { if (target?.isConnected === true) target.focus({ preventScroll: true }) })
  }
