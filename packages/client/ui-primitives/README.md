...
With AppFrame mounted, `Modal` participates in document-wide focus isolation. A body-portaled `Menu` identifies its anchor through `data-dsh-portal-owner` so the layout keeps that menu interactive while the background is inert. Escape closes the menu before its containing dialog; a nested modal does not also close its parent. Outside AppFrame, consumers remain responsible for focus isolation.
