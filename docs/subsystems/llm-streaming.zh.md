...
- **被截断的流和空 completion 都是可重试错误，而不是静默的成功结果。** 两个适配器都把提供方在终止标记前关闭的流标记为 `STREAM_CLOSED`，并把没有携带任何内容块的终止性 `stop` 结束映射为携带规范 `EMPTY_RESPONSE` code 的 `finish {kind:'error'}`；normal 模式的 `dsh-llm-retry` 默认会重试这两个 code。
