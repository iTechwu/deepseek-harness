...
挂载 AppFrame 后，`Modal` 参与 document 级焦点隔离。通过 portal 挂载到 body 的 `Menu` 使用 `data-dsh-portal-owner` 标识锚点，使布局在背景处于 inert 状态时仍保留该菜单的交互。Escape 优先关闭菜单，不关闭包含它的对话框；嵌套弹窗也不会同时关闭父弹窗。在 AppFrame 之外，消费方仍需负责焦点隔离。
