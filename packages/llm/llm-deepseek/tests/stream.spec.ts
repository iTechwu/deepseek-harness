...
describe('SSE framing and provider failures', () => {
  ...
  it.each([
    ...
    [400, {}, 'INVALID_REQUEST'], [413, {}, 'REQUEST_BODY_TOO_LARGE'], [503, {}, 'SERVER'], [404, {}, 'HTTP_404'],
  ...
  it('names the gateway byte limit on a 413 without a provider message', () => {
    const failure = providerError({ error: {} }, 413)
    expect(failure.failure.message).toBe('DeepSeek request body exceeds the gateway byte limit (HTTP 413)')
    expect(providerError({ error: { message: 'payload too large' } }, 413).failure.message).toBe('payload too large')
  })
