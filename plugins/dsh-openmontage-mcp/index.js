...
5. 严格按阶段执行客户端生命周期：对每个要由 Agent 完成的 stage，先调用 mcp__openmontage__begin_client_stage；它返回 jobId、stage、stageAttempt 和 leaseToken。后续工具使用 snake_case 参数，必须映射为 job_id=jobId、stage=stage、stage_attempt=stageAttempt、lease_token=leaseToken，并在顶层增加非空稳定 idempotency_key。然后读取该阶段指令与 tools_available：在 begin 和 submit 之间，只调用该阶段明确允许的工具零次或多次；tools_available 为空时不要虚构调用。所有 mcp__openmontage__invoke_openmontage_tool 的 generate / preflight / rank / progress 调用都必须携带这五项，不能只传 tool_name、operation、inputs。阶段结束后用同一组 lease 信息调用 mcp__openmontage__submit_client_stage；其 artifacts 必须以阶段标准产物名为 key，例如 research 阶段传 {"research_brief": {"version":"1.0", ...}}，不能把 version、topic 等 brief 字段直接放在 artifacts 顶层。不要脱离 begin/submit 生命周期直接调用 selector 或 compose 工具。调用骨架：submit_video_job → begin_client_stage →（按 tools_available 调用零次或多次 invoke_openmontage_tool）→ submit_client_stage。
