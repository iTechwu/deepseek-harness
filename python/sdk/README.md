...
Session control is available at both layers: `Session.cancel()` maps to `session/cancel`, `Session.resume()` restores a persisted session, and `Session.set_approval_policy("ask"|"never")` appends the durable policy event. `DeepSeekHarness.run(..., environment={"DEEPSEEK_BASE_URL": "https://..."})` passes an immutable, non-secret per-session endpoint overlay; the server rejects credentials and dynamic-loader variables. Individual approval prompts still require a future server-to-client request/response extension.
